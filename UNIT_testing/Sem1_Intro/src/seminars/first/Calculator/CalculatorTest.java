...
//        ).isInstanceOf(IllegalStateException.class);
        ...
        // Тестирование метода расчета скидки
        assertThat(Calculator.calculateDiscount(1000, 10)).isEqualTo(900);
        assertThat(Calculator.calculateDiscount(500, 0)).isEqualTo(500);
        assertThat(Calculator.calculateDiscount(100, 100)).isEqualTo(0);

        // Проверка недопустимых аргументов с использованием AssertJ
        assertThatThrownBy(() -> Calculator.calculateDiscount(-100, 10))
                .isInstanceOf(ArithmeticException.class)
                .hasMessage("Invalid discount or purchase amount");
        ...
        // Проверка выброса исключения при неверном операторе
        assertThatThrownBy(() -> Calculator.calculation(8, 4, '_'))
                .isInstanceOf(IllegalStateException.class)
                .hasMessage("Unexpected value operator: _");
    }
}